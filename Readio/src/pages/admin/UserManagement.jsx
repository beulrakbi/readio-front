...
//                 userTypes: selectedUserTypes.length > 0 ? selectedUserTypes : undefined,
...
//         // 여기서 setCurrentPage(1) 하고 바로 fetchUserList() 호출하면, 
//         // 상태 currentPage가 아직 1로 바뀌기 전에 fetchUserList가 호출돼서 이전 페이지가 쿼리될 수 있어서 주석
//         // fetchUserList();
//         fetchUserListWithPage(1); // 새로운 함수로 page=1로 강제 호출
//         console.log({ searchType, searchValue, startDate, endDate, userTypes, reportStatus });
//     };

//     const fetchUserListWithPage = async (page) => {
//     try {
//         const selectedUserTypes = Object.entries(userTypes)
//             .filter(([_, checked]) => checked)
//             .map(([type]) => type);
                ...
                <tbody>
                    {users.length > 0 ? (
                        users.map((user, index) => (
                            <tr key={user.userId}>
                                <td>{(currentPage - 1) * 10 + index + 1}</td>
                                <td><span onClick={() => handleUserClick(user)} className={styles.userIdClickable}>{user.userId}</span></td>
                                <td>{user.userEmail}</td>
                                <td>{formatToKST(user.userEnrollDate)}</td>
                                <td>{user.userRoleName}</td>
                                <td>{user.reportCount}</td>
                                <td>
                                    <select value={user.userRoleName} onChange={(e) => handleRoleChange(user.userId, e.target.value)} className={styles.roleSelect}>
                                        <option value="일반회원">일반회원</option>
                                        <option value="정지회원">정지회원</option>
                                        <option value="관리자">관리자</option>
                                    </select>
