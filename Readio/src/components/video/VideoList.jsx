...
function VideoList({type, userCoords, userId}) {
    ...
    if (!userId && loginRequired.includes(type.typeId) || type.typeId === 7) {
        ...
        if (type.typeId === 5) {
            ...
            try {
                ...
                for (let kw of keywords) {
                    ...
                    const dbList = Array.isArray(dbRes?.videoDTOList) ? dbRes.videoDTOList.filter((v, i, self) => i === self.findIndex(x => x.videoId === v.videoId)) : [];
                    allDB.push(...dbList);

                    // 유튜브 API 조회 (DB에 없는 만큼)
                    const numInDB = dbList.length;
                    const ytList = await getNewVideos(type.typeId, kw + '도서', dispatch, numInDB, dbList, filters) || [];
                    allYt.push(...ytList);
                }
